...
// buildIncrement, versionNumber, releaseDate, windowsReleaseName, windowsReleaseHash, linuxReleaseName, linuxReleaseHash, comment
const builds = [
    ...
    [52,  2789, "2025-11-14", "",                                                                        null, null,                            null, null],
    ...
    [108, 3640, "2026-02-20", null,                            null, "setup_ksa_v2026.2.31.3640.tar.gz", null, null,                            null, "no windows build due to linux needing an immediate rebuild"],
...
let reversedBuilds = builds.reverse();


reversedBuilds.forEach(([buildIncrement, versionNumber, releaseDate, windowsReleaseName, windowsReleaseHash, linuxReleaseName, linuxReleaseHash, comment]) => {
    const tr = document.createElement("tr");
    const missingRelease = `<span class="no-version">missing</span>`

    if (!windowsReleaseName && !linuxReleaseName)
    {
        tr.classList.add("missing");
    }

    const windowsRelease = windowsReleaseName
        ? `<a class="dl-link" href="${R2}/${buildIncrement}/${windowsReleaseName}">↓ Windows</a>`
        : missingRelease;

    const linuxRelease = linuxReleaseName
        ? `<a class="dl-link linux" href="${R2}/${buildIncrement}/${linuxReleaseName}">↓ Linux</a>`
        : missingRelease;


    // #, date, notes, windows, linux, windows hash, linux hash
    tr.innerHTML = `
    <td class="ver-num">${versionNumber}</td>
    <td class="build-num">${buildIncrement}</td>
    <td class="build-date">${releaseDate}</td>
    <td>${comment ? `<span class="comment-tag">${comment}</span>` : ""}</td>
    <td>${windowsRelease}</td>
    <td class="hash-cell">${windowsReleaseHash ? "No hash provided" : "-"}</td>
    <td>${linuxRelease}</td>
    <td class="hash-cell">${linuxReleaseHash ? "No hash provided" : "-"}</td>`;
